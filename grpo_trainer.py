...
class GRPOTrainer(RLHFTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def _generate_and_score_completions(self, inputs: InputsType) -> InputsType:
        ...
        if self.args.dynamic_sample and mode == 'train':
            ...
            inputs, total_rewards, total_rewards_per_func, completions,inverse_alpha = \
        ...
        batch_encoded_inputs = self._prepare_batch_inputs(inputs, total_rewards, inverse_alpha)
        # Log metrics
        messages = [inputs[i]['messages'][:-1] for i in range(len(inputs))]
        solution = [inp.get('solution', '') for inp in inputs]

        self._log_metrics(batch_encoded_inputs, messages, completions, solution, total_rewards, total_rewards_per_func)
    ...
    def _dynamic_sampling(self, inputs, rewards, rewards_per_func, completions):
        ...
        final_grouped_rewards = rewards.view(-1, self.num_generations)
    
        # 获取最终批次的总组数
        num_total_groups = final_grouped_rewards.shape[0]

        if num_total_groups > 0:
            # 计算最终批次中每个组的标准差
            final_group_std = final_grouped_rewards.std(dim=1)
            # 计算有效组的数量 (标准差 > 0)
            num_valid_groups = (final_group_std > 0).sum().item()
            # 计算 inverse_alpha = 有效组数 / 总组数
            inverse_alpha = num_valid_groups / num_total_groups
        else:
            # 如果批次为空，则认为所有样本都有效，避免除以零
            inverse_alpha = 1.0

        return inputs, rewards, rewards_per_func, completions, inverse_alpha

    def _prepare_batch_inputs(self, inputs: InputsType, rewards: torch.Tensor, inverse_alpha: float) -> InputsType:
        ...
        for i, mini_batch_advantage in enumerate(mini_batch_advantages):
            ...
            batch_encoded_inputs[i]['inverse_alpha'] = inverse_alpha

        return batch_encoded_inputs

    def _log_metrics(self, inputs, messages, completions, solution, rewards, rewards_per_func):
        ...
        # print(inputs)
        # self._textual_logs['image_paths'].append(gather_object(inputs['image_paths']))
        ...
        self._textual_logs['solution'].extend(gather_object(solution))
    ...
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        # 计算批次中所有有效 token 的平均损失
        loss = (per_token_loss * completion_mask).sum() / completions_length

        # GPG 修改: 应用梯度修正因子 α
        # alpha = 1 / inverse_alpha = B / (B-M)
        # 添加 1e-8 以防止除以零的错误
        alpha = 1.0 / (inputs["inverse_alpha"] + 1e-8)
        loss = loss * alpha
        ...
        # GPG 修改: 移除 PPO 特有的 clip_ratio 度量
        # is_clipped = ...
        # clip_ratio = ...
        ...
        if return_outputs:
            ...
            # GPG 修改: 也返回 alpha 值，方便监控
            metrics['gpg_alpha'] = alpha
            return loss, metrics
        else:
            for key, value in metrics.items():
                self._metrics[mode][key].append(self.accelerator.gather_for_metrics(value).mean().item())
            # GPG 修改: 也记录 alpha 值
            self._metrics[mode]['gpg_alpha'].append(alpha)
            return loss
    ...
    def log(self, logs: dict[str, float], start_time: Optional[float] = None) -> None:
        ...
        if self.accelerator.is_main_process and self.log_completions:
            table = {
                ...
                'solution': self._textual_logs['solution'],  # Add this line
